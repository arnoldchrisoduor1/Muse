...
const Page = () => {
  const { user, updateProfile, uploadAvatar, isLoading, logout, isAuthenticated } = useUserStore();
    ...
    <>
      <div className="w-[95%] max-w-3xl mx-auto py-6">
        <div className="w-full flex flex-col items-center justify-center mx-auto">
          {/* Profile Image */}
          <div className="relative mb-4 group cursor-pointer" onClick={handleProfileImageClick}>
            <Image
              src={getAvatarSrc()}
              width={120}
              height={120}
          ...
          <div className="w-full max-w-md bg-white rounded-xl shadow-sm p-6 mb-8">
            ...
            <div className="flex justify-center gap-2 items-center mb-4">
              <p className="text-xl font-semibold">{user.first_name || "First"}</p>
              <p className="text-xl font-semibold">{user.last_name || "Last"}</p>
            </div>
            
            {/* Bio */}
            <div className="text-center mb-6">
              <p className="text-gray-600 italic">{user.profile?.bio || "No bio yet"}</p>
            </div>
            
            {/* Stats */}
            <div className="flex justify-center gap-8 mb-6">
              <div className="text-center">
                <p className="text-2xl font-bold text-gray-800">{user.stats?.poem_count}</p>
                <p className="text-sm text-gray-500">Written</p>
              </div>
              <div className="text-center">
                <p className="text-2xl font-bold text-gray-800">{user.stats?.total_likes}</p>
              ...
              {[1, 2, 3].map((item) => (
                <div key={item} className="bg-white p-4 rounded-lg shadow-sm hover:shadow-md transition-shadow">
                  <div className="flex justify-between items-start">
                    <h3 className="font-semibold text-lg mb-2">Title of poem</h3>
                    <span className="text-xs text-gray-500">3 days ago</span>
                  </div>
                  <p className="text-gray-600 text-sm">Description of the poetry that's been written</p>
                  <div className="flex items-center gap-2 mt-2">
                    <span className="text-xs bg-indigo-100 text-indigo-800 px-2 py-1 rounded-full">Poetry</span>
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex flex-col items-center mb-4">
                    ...
                    <p className="text-sm text-gray-500">
                      {formData.avatar ? formData.avatar.name : "Click to select a new profile image"}
                    </p>
                  </div>
